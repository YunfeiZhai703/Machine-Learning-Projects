...
def eprank(G, M, num_iters):
    ...
    for iter in range(num_iters):
        for p in range(M):  # compute marginal player skills
            ...
            temp_Ps = 1./pv + np.sum(Pgs[games_won, 0]) + np.sum(Pgs[games_lost, 1])
            temp_Ms = (np.sum(Pgs[games_won, 0] * Mgs[games_won, 0]) / temp_Ps) \
                + (np.sum(Pgs[games_lost, 1] * Mgs[games_lost, 1]) / temp_Ps)

            # Store the current iteration values for player p
            Ms[p, iter] = temp_Ms
            Ps[p, iter] = temp_Ps
